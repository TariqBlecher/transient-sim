...
class Transient:
    ...
    def __post_init__(self):
        ...
        if self.periodic and self.period_sec is None:
            raise ValueError(f"Transient '{self.name}': period_sec must be set when periodic=True")
...
def save_transients_yaml(transients: List[Transient], output_path: Union[str, Path]) -> Path:
